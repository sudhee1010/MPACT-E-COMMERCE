...
export default function CarouselVideosAdmin() {
  ...
  const handleYouTubeSubmit = () => {
    ...
    const videoId = extractYouTubeIdOld(youtubeUrl);
    if (!videoId) {
  ...
  const openEditModal = (video) => {
    ...
    setYoutubeUrl("");
    setSelectedFile(null);
    setVideoSource(video.videoUrl?.includes('youtube') ? 'youtube' : 'upload');
    setSearchTerm(video.productName || "");
    setShowDropdown(false);
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.videoUrl) {
      alert("Please add a video (YouTube URL or upload)");
      return;
    }
  ...
  // Demo YouTube URLs
  const demoYouTubeUrls = [
    { name: "Product Demo 1", url: "https://www.youtube.com/watch?v=dQw4w9WgXcQ" },
    { name: "Product Demo 2", url: "https://www.youtube.com/watch?v=3JZ_D3ELwOQ" },
    { name: "Product Demo 3", url: "https://www.youtube.com/watch?v=JGwWNGJdvx8" },
  ];
      ...
      <div className="carousel-admin">
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <h3>No product videos yet</h3>
            <p>Click "Add Video to Product" to get started</p>
            <p style={{ fontSize: '14px', marginTop: '10px', color: '#facc15' }}>
              💡 Tip: Use YouTube URLs for demo videos
            </p>
          </div>
        ) : (
          <div className="videos-grid">
              ...
              <form onSubmit={handleSubmit}>
                <div className="modal-body">
                  ...
                  <div className="video-source-tabs">
                    ...
                    <button
                      type="button"
                      className={`source-tab ${videoSource === 'upload' ? 'active' : ''}`}
                    ...
                    {videoSource === 'youtube' ? (
                      <>
                        ...
                        <div className="demo-urls">
                          <span style={{ color: '#999', fontSize: '12px' }}>Demo URLs:</span>
                          {demoYouTubeUrls.map((demo, index) => (
                            <button
                              key={index}
                              type="button"
                              ...
                              onClick={() => {
                                ...
                                setTimeout(() => handleYouTubeSubmit(), 100);
                              }}
                            >
                              {demo.name}
                            </button>
                      ...
                      <div style={{ marginTop: '20px' }}>
                        ...
                        <div className="video-preview-container">
                          {formData.videoUrl.includes('youtube') ? (
                            <iframe
                              src={`${formData.videoUrl}?autoplay=1&mute=1&loop=1&playlist=${extractYouTubeId(formData.videoUrl)}&controls=1&modestbranding=1&rel=0`}
                              title="Video preview"
                              frameBorder="0"
                              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                              allowFullScreen
