...
export const adminDownloadInvoice = async (req, res) => {
  try {
    ...
    doc.fontSize(18).text("TAX INVOICE", 0, 40, { align: "right" });
    doc
      .fontSize(10)
      .text(`Invoice No: ${invoiceNumber}`, { align: "right" })
    ...
    doc
      .fontSize(10)
      .text(order.shippingAddress.name || order.user?.name || "Customer")
      .text(order.shippingAddress.address || "—")
      .text(
        `${order.shippingAddress.city || "—"}, ${order.shippingAddress.state || ""} - ${order.shippingAddress.pincode || "—"}`
      )
    ...
    order.orderItems.forEach((item) => {
      doc
        ...
        .text(item.quantity, qtyX, position)
        .text(`RS: ${item.price.toFixed(2)}`, priceX, position)
        .text(`RS: ${(item.price * item.quantity).toFixed(2)}`, totalX, position);
      position += 18;
    });
    ...
    position += 15;
    doc.fontSize(10).text(`Subtotal: RS: ${subtotal.toFixed(2)}`, 350, position);
    position += 16;
    doc.text(`CGST (9%): RS: ${cgst.toFixed(2)}`, 350, position);
    position += 16;
    doc.text(`SGST (9%): RS: ${sgst.toFixed(2)}`, 350, position);
    ...
    doc
      .fontSize(12)
      .text(`Grand Total: RS: ${order.totalAmount.toFixed(2)}`, 350, position, { underline: true });
